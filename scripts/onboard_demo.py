...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"    meta-train tasks={len(meta_tasks)}  holdout tasks={len(holdout_tasks)}"
    ...
    print(
        f"    query RMSE = {before['mean_query_rmse_days']:.1f} days   "
    ...
    print(
        f"    query RMSE = {after['mean_query_rmse_days']:.1f} days   "
        f"early-warning acc = {after['mean_early_warning_accuracy']:.3f}"
    )
    gain = (
        100.0
        * (before["mean_query_rmse_days"] - after["mean_query_rmse_days"])
    ...
    adapted, report = hermes.onboard(
        ...
        support_x=new.support_x,
        support_y=new.support_y,
        unlabeled_x=pool_x,
        eval_x=eval_x,
        eval_y=eval_y,
    )
    ...
    if args.export:
        ...
        payload = TurbinePayload(
            asset_id="turbine-NEW-001",
            telemetry=Telemetry(
                **{c: float(snap[c]) for c in holdout_df.columns if c != "timestamp"}
            ),
        )
        for name, ckpt in (("meta", meta_bundle.checkpoint_path), ("adapted", paths["checkpoint"])):
            ...
            print(
                f"    [{name}] serving advisory OK: RUL={rec['predicted_rul_days']:.1f} d "
