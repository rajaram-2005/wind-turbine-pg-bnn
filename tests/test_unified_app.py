...
def test_unified_health_discovers_every_surface():
    with TestClient(create_app(include_dashboard=False)) as client:
        ...
        assert body["services"]["model_inference"] == "/api/model"
        assert body["services"]["model_stream"] == "/api/model/stream"
        # Consolidated deployment: no separate model app or legacy dashboard.
        assert "model_api" not in body["services"]
        assert "legacy_dashboard" not in body["services"]
...
def test_model_surface_is_initialized_on_the_single_api():
    with TestClient(create_app(include_dashboard=False)) as client:
...
def test_legacy_surfaces_are_gone():
    with TestClient(create_app()) as client:
        # The old /legacy redirect and /model-api mount were consolidated away.
        assert client.get("/legacy").status_code == 404
        assert client.get("/model-api/health").status_code == 404
